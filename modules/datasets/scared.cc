...
Scared::Scared(const std::string &video_path, const std::string& other_video_path) {
...
absl::StatusOr<cv::Mat> Scared::GetImage(const int idx) {
...
absl::StatusOr<cv::Mat> Scared::GetRightImage(const int idx) {
...
bool Scared::SplitVideoIntoFrames(const std::string &path, const std::string &video_path,
    ...
    // Use this for Scared 01.01.
    leftCal_ = (cv::Mat_<double>(3,3) << 1035.30811, 0.0, 596.955017,
            0.0, 1035.08765, 520.410034,
            0.0, 0.0, 1.0);
    leftDistorsion_ = (cv::Mat_<double>(1,5) << -0.000595157442, -0.000546629308, 0.0, 0.0, 0.00182959007);

    rightCal_ = (cv::Mat_<double>(3,3) << 1035.17419, 0.0, 688.361877,
            0.0, 1034.979, 521.070801,
            0.0, 0.0, 1.0);
    rightDistorsion_ = (cv::Mat_<double>(1,5) << -0.000234280655, -0.000768933969, 0.0, 0.0, 0.000776395318);

    R = (cv::Mat_<double>(3,3) << 1.0, 1.94856493e-05, -0.000152324792,
          -1.95053162e-05, 1.0, -0.000129114138,
          0.000152322275, 0.000129117107, 1.0);
    t = (cv::Mat_<double>(3,1) << -4.14339018,
            -0.0238197036,
            -0.00190685259);
